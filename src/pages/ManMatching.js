...
export default function ManMatching() {
  ...
  const setImage = async () =>{
    axios.get('http://143.248.192.51:8080/auth/random').then(result => {
      console.log(result.data);
        const resArr = [ result.data.id_1, result.data.id_2, result.data.id_3, result.data.id_4 ];
        setIdList(resArr)
        console.log(resArr);
        Promise.all(
          resArr.map((e) => axios.get((`http://143.248.192.51:8080/api/download?user_id=${e}`), { responseType: 'arraybuffer'})
        )).then(res => {
          setImages(res.map((e) => {
